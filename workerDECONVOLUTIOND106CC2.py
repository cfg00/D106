...
save_folder = r'/mnt/merfish9/20230908_D106Luo/DNA/AnalysisDeconvolveCG'
...
def main_f(set_ifov,try_mode=True):
   ...
   # r'\\merfish6\merfish6v2\DNA_FISH\20230212_R120PVTS32RDNABDNF\'
    ...
    all_flds_ = glob.glob(r'/mnt/merfish9/20230908_D106Luo/DNA/H*B*'+set__) ################################# modify for 3-col small tad 
    redefine_color += [[0,1,2,3]]*len(all_flds_) #### three signal color
    all_flds+=all_flds_

    all_flds = [s for s in all_flds if "H54" not in s]

    all_flds = [s for s in all_flds if "H53" not in s]
    #all_flds = [s for s in all_flds if "H38" not in s]
    #all_flds = [s for s in all_flds if "H39" not in s]
    #all_flds = [s for s in all_flds if "H40" not in s]
    #all_flds = [s for s in all_flds if "H41" not in s]
    #all_flds = [s for s in all_flds if "H42" not in s]
    #all_flds = [s for s in all_flds if "H43" not in s]
    #all_flds = [s for s in all_flds if "H44" not in s]
    all_flds = [s for s in all_flds if "H45" not in s]
    #all_flds = [s for s in all_flds if "H46" not in s]
    #all_flds = [s for s in all_flds if "H47" not in s]
    all_flds = [s for s in all_flds if "H70" not in s]
